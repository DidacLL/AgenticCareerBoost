...
for (const route of routes) {
  ...
  if (canonical !== expectedCanonical) {
    throw new Error(`Canonical mismatch on ${route}: expected ${expectedCanonical}, got ${canonical ?? "missing"}`);
  }
  if (indexable === hasNoindex(html)) {
    throw new Error(`Robots metadata mismatch on ${route}: indexable=${indexable}`);
  }
...
if (!/<main[ >]/i.test(notFoundHtml) || !hasNoindex(notFoundHtml)) {
  throw new Error("404 page must render the site shell and remain noindex.");
}
...
if (indexable) {
  if (!/Allow:\s*\//i.test(robots) || /Disallow:\s*\//i.test(robots)) {
    throw new Error("Indexable build robots.txt must allow crawling.");
  }
