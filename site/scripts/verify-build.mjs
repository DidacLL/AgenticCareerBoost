...
if (!outputArg) {
  throw new Error("Usage: node verify-build.mjs <dist> [base]");
}
...
for (const file of walk(resolve("site/src"))) {
  ...
  if (
    source.includes("https://didacll.github.io") ||
    source.includes("http://didacll.github.io") ||
    source.includes('href="/AgenticCareerBoost/') ||
    source.includes('src="/AgenticCareerBoost/')
  ) {
...
function pageFile(route) {
  return join(output, route === "/" ? "index.html" : `${route.replace(/^\//, "")}index.html`);
}

function references(html) {
  return [...html.matchAll(/(?:href|src)=["']([^"'#]+)["']/gi)].map((match) => match[1]);
}
...
for (const route of routes) {
  ...
  for (const required of [
    ...
    if (!required.test(html)) {
      throw new Error(`Missing page metadata or main region: ${route}`);
    }
  }

  for (const reference of references(html)) {
    if (/^(?:https?:|mailto:|tel:|data:|#)/i.test(reference) || /\/files\/cv\//.test(reference)) {
      continue;
    }
...
console.log(`Verified ${routes.length} current portfolio routes for base ${base}`);
