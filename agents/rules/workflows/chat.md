...
- `agents/rules/core/*` — mission, brand, constraints (read-only)
- `agents/state/current.md` — current status for context

## Steps

1. Load stable truth from `agents/rules/core/*` and current state.
...
4. Optionally write the summary to
   `agents/state/summaries/YYYY-MM-DD-chat-<slug>.md`.

## Outputs

- Conversation responses respecting project tone and constraints
- Optional: session summary file in `agents/state/summaries/`
