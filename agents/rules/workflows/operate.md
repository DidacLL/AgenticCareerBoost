...
- `agents/rules/core/execution-modes.md` — selected mode and negative scopes
- `agents/rules/core/constraints.md` — bounded contract fields
- `agents/rules/core/*` — stable truth (read-only)
- One role file from `agents/rules/roles/` or `agents/rules/roles/autoagents.md`
- `agents/state/current.md` — current project status
- One declared family memory path or `none`

## Steps

1. Select the execution mode from `agents/rules/core/execution-modes.md`.
2. Write one bounded contract using `agents/rules/core/constraints.md`.
