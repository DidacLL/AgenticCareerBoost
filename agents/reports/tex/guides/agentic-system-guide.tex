...
\pathref{site/files/reports/s000-agentic-os-bootstrap.pdf}.
...
The canonical mission lives in \pathref{agents/rules/core/mission.md}. In practical
...
\begin{tikzpicture}[node distance=1.5cm and 2.1cm]
  ...
  \node[filenode, below left=1.6cm and 2.7cm of entry] (core) {agents/rules/core/};
  \node[agentnode, below=of entry] (workflows) {agents/rules/workflows/};
  \node[agentnode, below right=1.6cm and 2.7cm of entry] (roles) {agents/rules/roles/};

  \node[statenode, below left=1.8cm and 1.5cm of workflows] (state) {agents/state/};
  \node[filenode, below=of workflows] (reports) {agents/reports/};
...
Stable truth lives under \pathref{agents/rules/core/}. These files describe the mission,
...
Volatile state lives under \pathref{agents/state/}. It holds the current status,
...
Outputs live under \pathref{content/}, \pathref{site/}, and
...
The fixed routines currently live in \pathref{agents/rules/roles/autoagents.md}. They
cover consistency, Markdown quality, LaTeX compilation, and social planning.

\section{Family memory}

Family memory lives in \pathref{agents/state/memory/}. It stores durable heuristics
such as recurring build issues, accepted phrasing defaults, or source-quality
notes. It must never replace canonical truth in \pathref{agents/rules/core/}.
...
\pathref{agents/rules/core/} & Understand what is stable and non-negotiable \\
\pathref{agents/rules/workflows/} & Choose the correct action mode \\
\pathref{agents/rules/roles/} & Understand who owns the work \\
\pathref{agents/state/} & Read current status, backlog, and recent activity \\
\pathref{agents/reports/} & Read published formal documentation \\
\pathref{site/} & Inspect the public mirror source \\
\bottomrule
\end{tabularx}
...
\begin{tikzpicture}[node distance=1.4cm and 1.8cm]
  ...
  \node[filenode, below=of guide] (site) {site/};
  \node[filenode, below=of case] (build) {site/files/reports/};
...
\begin{description}[style=nextline]
  \item[What is always true?] Read \pathref{agents/rules/core/}.
  \item[What action should be used?] Read \pathref{agents/rules/workflows/}.
  \item[Who owns each kind of work?] Read \pathref{agents/rules/roles/}.
  \item[What changed recently?] Read \pathref{agents/state/current.md} and the
  backlog.
  \item[How was the bootstrap system implemented?] Read the S-000 case study.
  \item[How are formal PDFs built?] Read \pathref{agents/reports/tex/README.md}
...
\pathref{agents/rules/core/mission.md} & purpose and success criteria \\
\pathref{agents/rules/workflows/review.md} & maintenance review chain \\
\pathref{agents/rules/workflows/operate.md} & one bounded specialist task \\
\pathref{agents/rules/roles/autoagents.md} & recurring routines and triggers \\
\pathref{agents/state/current.md} & current status snapshot \\
\pathref{site/files/reports/agentic-system-guide.pdf} & published human-facing guide \\
\pathref{site/files/reports/s000-agentic-os-bootstrap.pdf} & published technical case study \\
\bottomrule
\end{longtable}
