...
\begin{tikzpicture}[node distance=1.5cm and 2cm]
  ...
  \node[agentnode, below left=1.5cm and 2.5cm of agents] (wf) {agents/rules/workflows/};
  \node[filenode, below right=1.5cm and 2.5cm of agents] (core) {agents/rules/core/};
  \node[agentnode, below=of wf] (roles) {agents/rules/roles/};
  \node[filenode, below=of roles] (templ) {agents/rules/templates/};
  \node[statenode, right=2.5cm of roles] (state) {agents/state/};
  \node[filenode, below=of templ] (outputs) {content/};
  \node[statenode, left=2cm of outputs] (logs) {agents/state/logs/};
  \node[agentnode, below=of outputs] (ci) {.github/workflows/};
  \node[filenode, below left=1.2cm and 1cm of ci] (status) {data/};
  \node[filenode, below right=1.2cm and 1cm of ci] (site) {site/};
...
2 & \pathref{agents/rules/core/} & Stable --- change only via system-review \\
3 & \pathref{agents/rules/workflows/} & Stable --- change only via system-review \\
4 & \pathref{agents/rules/roles/} & Stable --- change only via system-review \\
5 & \pathref{agents/state/} & Volatile --- updated every sprint/hotfix \\
...
only \pathref{agents/rules/core/} and \pathref{agents/state/current.md} --- under 3\,KB of
...
% ============================================================================
\chapter{Canonical Truth: agents/rules/core/}
\label{ch:core}
% ============================================================================
...
\pathref{agents/rules/core/mission.md} states the project goal, scope, success
...
\pathref{agents/rules/core/brand.md} defines positioning and tone constraints.
...
\pathref{agents/rules/core/marketing.md} establishes the campaign strategy.
...
\pathref{agents/rules/core/constraints.md} lists hard and soft constraints.
...
\pathref{agents/rules/core/truth-hierarchy.md} is the canonical source for the
...
\pathref{agents/rules/core/tool-policy.md} lists the approved toolchain: GitHub
...
% ============================================================================
\chapter{Workflow Contracts: agents/rules/workflows/}
\label{ch:workflows}
% ============================================================================
...
Every workflow file under \pathref{agents/rules/workflows/} uses a shared five-section
...
\pathref{agents/state/roadmap.md}; decompose into tasks with owning role, acceptance
criteria, and expected output; define pair-check and backlog requirements; fill
the sprint-output template; copy to \pathref{agents/state/active-sprint.md}; update
...
\textbf{Trigger:} A populated \pathref{agents/state/active-sprint.md} with
...
\begin{enumerate}
  ...
  \item Public-narrative decision.
...
Read \pathref{agents/rules/core/} and \pathref{agents/state/current.md}. No file edits except
an optional summary written to \fname{agents/state/summaries/}. Chat never silently
...
% ============================================================================
\chapter{Agent Roles: agents/rules/roles/}
\label{ch:agents}
% ============================================================================
...
% ============================================================================
\chapter{Output Templates: agents/rules/templates/}
\label{ch:templates}
% ============================================================================
...
\pathref{agents/rules/templates/sprint-output.md} structures a sprint plan and closure
...
The filled template is copied to \pathref{agents/state/active-sprint.md} when a sprint
is planned, and updated in place as tasks complete.

\section{paircheck-output.md}

\pathref{agents/rules/templates/paircheck-output.md} records an independent review
...
Two filled instances are saved to \fname{agents/state/logs/} per reviewed output.

\section{documentation-output.md}

\pathref{agents/rules/templates/documentation-output.md} accompanies every documentation
...
\pathref{agents/rules/templates/community-output.md} structures every public-facing
...
\pathref{agents/rules/core/brand.md}, evidence linking, cross-channel originality, and
...
instance saved in \fname{agents/state/logs/}.}


% ============================================================================
\chapter{State Machinery: agents/state/}
\label{ch:state}
% ============================================================================
...
subordinate to stable truth in \pathref{agents/rules/core/}.}
...
\pathref{agents/state/current.md} is a one-screen snapshot: active workflow, active
...
\pathref{agents/state/active-sprint.md} holds the current sprint contract (filled from
\pathref{agents/rules/templates/sprint-output.md}). When no sprint is active, it reads
\texttt{status: idle}. Task status is updated in place as work progresses.

\section{roadmap.md}

\pathref{agents/state/roadmap.md} lists 3--5 upcoming sprint seeds. Each seed becomes
a full plan via the Plan workflow. Seeds are consumed (moved to active) and
replenished after system reviews.

\section{backlog.md}

\pathref{agents/state/backlog.md} has two sections: \emph{Technical backlog} (tickets
...
\fname{agents/state/logs/} accumulates pair-check outputs, CI traces, and per-sprint
evidence files. \fname{agents/state/summaries/} stores chat session summaries and
periodic digests. Both are append-only during a sprint.

\pitfall{Never treat data in \fname{agents/state/} as canonical. If a volatile file
says the mission is X but \pathref{agents/rules/core/mission.md} says Y, the core file
...
\begin{description}[style=nextline]
  ...
  \item[\pathref{site/}] Jekyll source for the public site.
  \item[\pathref{agents/reports/}] Formal \LaTeX{} engineering documents.
    This directory hosts the \fname{tex/} subtree with shared preamble and
    per-sprint documents.
\end{description}

\section{site/ --- Jekyll scaffold}

\pathref{site/} contains the canonical Jekyll site: \fname{\_config.yml}
...
\pathref{site/data/status.json} exports sprint status for the public site:
sprint ID, workflow, last closure timestamp, artifacts array, and blockers.
It is auto-generated by a GitHub Actions workflow whenever \pathref{agents/state/}
changes.
...
\pitfall{Do not edit \fname{site/data/status.json} by hand. It is
machine-generated by \fname{export-status.yml}. Manual edits will be
overwritten on the next push to \pathref{agents/state/}.}
...
Triggers on push to main affecting \fname{site/},
or \fname{README.md}. Uses the official GitHub Pages actions
(\texttt{actions/configure-pages}, \texttt{actions/jekyll-build-pages},
\texttt{actions/deploy-pages}) with source at \fname{./site/}.
Permissions: \texttt{pages: write}, \texttt{id-token: write}.

\subsection{export-status.yml}

Triggers on push to main affecting \fname{agents/state/} and on manual dispatch.
A Python script (\fname{.github/scripts/export\_status.py}, $\leq 40$ lines)
parses front-matter from \pathref{agents/state/current.md} and
\pathref{agents/state/active-sprint.md}, writes \pathref{site/data/status.json},
and auto-commits only if content changed.

\subsection{latex-build.yml}

Triggers on PR and push touching \fname{agents/reports/tex/}. Uses
...
\screenshotfig{figures/screenshots/screenshot001.png}%
...
\pathref{agents/state/} updated, no orphan work.

\pitfall{Adding a workflow that introduces a tool not listed in
\pathref{agents/rules/core/tool-policy.md} is a policy violation. The tool must be
...
\midrule
\pathref{agents/rules/core/mission.md} & Goal, scope, success criteria & $\sim$29 \\
\pathref{agents/rules/core/brand.md} & Positioning, tone, language policy & $\sim$36 \\
\pathref{agents/rules/core/marketing.md} & Campaign strategy & $\sim$33 \\
\pathref{agents/rules/core/constraints.md} & Hard and soft constraints & $\sim$23 \\
\pathref{agents/rules/core/truth-hierarchy.md} & Conflict resolution order & $\sim$20 \\
\pathref{agents/rules/core/tool-policy.md} & Approved toolchain & $\sim$24 \\
\midrule
\pathref{agents/rules/workflows/plan.md} & Sprint planning workflow & $\sim$37 \\
\pathref{agents/rules/workflows/sprint.md} & Sprint execution workflow & $\sim$42 \\
\pathref{agents/rules/workflows/hotfix.md} & Focused fix workflow & $\sim$35 \\
\pathref{agents/rules/workflows/chat.md} & Discussion workflow & $\sim$31 \\
\pathref{agents/rules/workflows/system-review.md} & System audit workflow & $\sim$34 \\
\midrule
\pathref{agents/rules/roles/orchestrator.md} & Orchestrator role contract & $\sim$36 \\
\pathref{agents/rules/roles/developer.md} & Developer role contract & $\sim$32 \\
\pathref{agents/rules/roles/paircheck.md} & PairCheck role contract & $\sim$38 \\
\pathref{agents/rules/roles/cicd.md} & CI/CD role contract & $\sim$34 \\
\pathref{agents/rules/roles/documentation.md} & Documentation role contract & $\sim$33 \\
\pathref{agents/rules/roles/community-manager.md} & CommunityManager role contract & $\sim$32 \\
\midrule
\pathref{agents/rules/templates/sprint-output.md} & Sprint plan/closure template & $\sim$48 \\
\pathref{agents/rules/templates/paircheck-output.md} & Review verdict template & $\sim$25 \\
\pathref{agents/rules/templates/documentation-output.md} & Documentation template & $\sim$29 \\
\pathref{agents/rules/templates/community-output.md} & Social artifact template & $\sim$26 \\
\midrule
\pathref{agents/state/current.md} & Active workflow snapshot & $\sim$12 \\
\pathref{agents/state/active-sprint.md} & Current sprint contract & $\sim$7 \\
\pathref{agents/state/roadmap.md} & Upcoming sprint seeds & $\sim$11 \\
\pathref{agents/state/backlog.md} & Technical + narrative backlog & $\sim$18 \\
\bottomrule
\end{longtable}

Additional infrastructure files (\fname{.github/}, \fname{site/},
