# Human actions

This file lists tasks that agents must not execute directly. The human owner
must review, decide, publish, contact, or apply manually.

## Before campaign relaunch

1. Review the public LinkedIn headline and About section.
   - Confirm they do not frame the profile as generic backend, data quality, BI,
     dashboarding, junior CRUD, or a primarily student profile.
   - Confirm they foreground agentic systems, systems thinking, ML/AI study,
     technical documentation, and regulated-domain maturity.

2. Review GitHub account metadata.
   - Confirm public website points to `https://didacll.github.io/`.
   - Confirm the public bio is aligned with `agents/rules/core/career-direction.md`.
   - Confirm pinned repositories prioritize AgenticCareerBoost, MADRE, P3CTeX,
     and one supporting engineering project.

3. Review old public surfaces.
   - Remove links to the 2022 `Didac-dev-project` site from controlled profiles.
   - Mark legacy repositories clearly, archive them, or keep them unpinned.
   - Remove stale `https://didacll.github.io/` links where the
     intended target is the main public site.
   - Keep AgenticCareerBoost references when the intended target is the project
     repository or project page.

4. Approve the first publication batch.
   - Pick three low-heat LinkedIn posts.
   - Confirm each has evidence.
   - Put links in the first comment or profile Featured area, not the post body.
   - Avoid identical copy across LinkedIn, site, and repository.

5. Decide the first company outreach batch.
   - Prefer product, research, AI, R&D, tooling, infrastructure, and regulated-domain companies.
   - Exclude body-shop consulting, unpaid internships, equity-only startups,
     relocation-required roles, and offers below the documented threshold.

## Publication checklist

For every post:

- [ ] Human approved exact final text.
- [ ] Evidence link exists.
- [ ] No AI-hype language.
- [ ] No self-pity or just-a-junior framing.
- [ ] No generic backend/data-quality positioning.
- [ ] No repeated student-first positioning outside CV/education context.
- [ ] Sarcastic edge, if present, targets systems or framings, not people.
- [ ] Call to action is specific, not engagement bait.

## Application checklist

For every application or proactive contact:

- [ ] Role passes `agents/rules/core/career-direction.md`.
- [ ] Company is not staff augmentation or CRUD outsourcing.
- [ ] The pitch is adapted to the company.
- [ ] The evidence link is chosen before writing the message.
- [ ] Salary, internship, relocation, and work-model constraints are checked.

## Human decision log

Use this section after manual action.

- `YYYY-MM-DD` — action taken — link/evidence — result.
