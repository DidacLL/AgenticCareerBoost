...
`import-seed` is intended for local/private migration files. Do not commit real application data or generated SQLite files. The dashboard loads application call cards, literal offer snapshots, company research, form answers, private/generated file references, and keyword definitions from the local database. Clicking a keyword chip opens its definition panel during recruiter calls.

The tracker supports partial-first records: create the application with company and role, paste the original offer later with `set-offer`, then let an agent complete company research, call cards, form answers, cover letters, interview guides, and file references in later passes.
